...
const embedGPSIntoImage = (
  ...
  try {
    ...
    try {
      ...
      if (!("thumbnail" in exifObj)) exifObj.thumbnail = undefined;
    } catch {
      exifObj = {
        "0th": {},
        "Exif": {},
...
// (Removed duplicate declaration of decimalToDMS, dmsToRational, and embedGPSIntoImage)
  ...
  const capturePhoto = (): void => {
    ...
    const processPhoto = async (photoDataUrl: string) => {
      ...
      try {
        ...
        if (locationData.latitude && locationData.longitude) {
          ...
          setPhotos(prev => [...prev, imageWithGPS]);
    ...
    if (canvas.toBlob) {
      canvas.toBlob(async (blob) => {
        ...
        await processPhoto(photoDataUrl);
      }, 'image/jpeg', 0.8);
    } else {
      const photoDataUrl = canvas.toDataURL('image/jpeg', 0.8);
